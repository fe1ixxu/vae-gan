...
## Preprocessed files:
Preprocessing has many steps, but we also have preprocessed files for sentiment transfer and code-switching transfer, which respectively located at `vae-fairseq/data/{all-yelp, all-en-hi}`.
...
Use command line:
```
fasttext test models/model.bin test.0
```
